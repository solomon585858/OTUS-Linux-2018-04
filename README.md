...
## Homework 6
 ...
 ## Homework 26. Postfix + Dovecot
 - В рамках ДЗ по **Postfix** и **Dovecot** на тестовой инфраструктуре *VMware* была поднята виртуальная машина с именем **mail.test.local**, на которой были развернуты **Postfix**, **Dovecot** и **MariaDB**. Логи установки и конфигурации можно посмотреть в файле **installation and configuration of postfix, dovecot and mariadb.txt**, который был выложен в папке **HW-26**. Логи создания тестовых пользователей и отправки тестового письма как с помощью **TELNET**, так и с помощью **mailx**, включая логи можно посмотреть в файле **sending test email.txt**, который был выложен в папке **HW-26**. Установку почтового клиента **mutt**, просмотр с помощью него тестовых писем, отправленных ранее, а также содержание писем с помощью **cat** можно посмотреть в файле **opening email with a client.txt**, который был выложен в папке **HW-26**.

 ## Homework 27. Простая связь через pjsip
 - В рамках ДЗ по **Asterisk** был поднят сервер **Asterisk** на виртуальной машине с помощью **ansible role** *https://github.com/erlong15/tls-asterisk14-ansible*. В рамках файла **pjsip.conf** был добавлен транспорт **transport-udp**. В рамках БД **asterisk** для номеров *1100-1102* был прописан **transport-udp**, а также отключен параметр **media_encryption**. На два компьютера было установлено приложение **MicroSIP**, на которых были добавлены номера *1100* и *1101* соответственно. По итогам был протестирован звонок между двумя телефонами с указанными номерами. Логи *pjsip* можно посмотреть в файле **pjsip.log**который был выложен в папке **HW-27**.
